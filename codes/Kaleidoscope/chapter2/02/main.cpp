...
static std::string GetTokenName( int tok )
{
    switch (tok) {
    case tok_identifier:
...
{
    CurTok = gettok();
    std::cout << "got token: " << GetTokenName( CurTok ) << "," << CurTok << "," << IdentifierStr << std::endl;
    if ( isascii( CurTok ) && isprint( CurTok ) )
    {
        std::cout << "CurTok=" <<  static_cast<char>(CurTok) << std::endl;
    }
    return CurTok;
}
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
    ...
    std::cout << "ParsePrototype : FnName = " << FnName << std::endl;
    getNextToken();
    ...
    while ( getNextToken() == tok_identifier )
    {
        ...
        std::cout << "ParsePrototype : ArgNames = " << IdentifierStr << std::endl;
    }
    if (CurTok != ')')
        return LogErrorP("Expected ')' in prototype");
